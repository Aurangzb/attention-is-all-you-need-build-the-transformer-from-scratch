...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    for word in specials:
        token_to_id_dictionary[word] = i
        i = i+1

    for items in sentences:
        words = items.split()
        for word in words:
            if word not in token_to_id_dictionary:
                token_to_id_dictionary[word] =  i
                i = i+1
...
def build_id_to_token_vocab(token_to_id):
    ...
    reversed_dict = {value: key for key, value in token_to_id.items()}
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for word in words:
        if word in token_to_id:
            sent_to_ids.append(token_to_id[word])
        else:
            sent_to_ids.append(token_to_id[unk_token])
...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    ids_to_token = []
    for i in ids:
        ids_to_token.append(id_to_token[i])
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    id_sequence_with_pad= []
    id_len = len(ids)
    if id_len < max_len:
        num_iter = max_len - id_len
        id_sequence_with_pad = ids
        for i in range(num_iter):
            id_sequence_with_pad.append(pad_id)
    else:
        id_sequence_with_pad = ids[:max_len]   
    
    return id_sequence_with_pad
